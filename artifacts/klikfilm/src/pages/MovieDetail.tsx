...
import { Play, Plus, Share2, Star, Clock, Globe2, AlertCircle, ArrowLeft, Video } from "lucide-react";
    ...
    return (
      <Layout>
        <div className="flex-1 flex flex-col items-center justify-center min-h-[70vh]">
          ...
          <p className="text-muted-foreground mb-6">The title you are looking for does not exist or has been removed.</p>
          <button 
            onClick={() => setLocation("/")}
                  ...
                  <button 
                    onClick={() => {
                      if(movie.streamUrls && movie.streamUrls.length > 0) {
                        ...
                        alert("No streaming URL available");
                      }
                    }}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-24">
          ...
          <div className="flex-1 space-y-10">
            ...
            {movie.director && (
              <section>
                <h3 className="text-lg font-display font-semibold text-foreground mb-2">Director</h3>
            ...
            {movie.genre && movie.genre.length > 0 && (
              <section>
                <h3 className="text-lg font-display font-semibold text-foreground mb-3">Genres</h3>
                <div className="flex flex-wrap gap-2">
                  {movie.genre.map((g) => (
                    <span key={g} className="px-4 py-2 rounded-lg bg-secondary border border-white/5 text-sm font-medium hover:border-primary/50 transition-colors cursor-pointer">
                      {g}
                    </span>
                  ))}
                </div>
            ...
            {movie.streamUrls && movie.streamUrls.length > 0 && (
              <section className="glass-panel p-6 rounded-2xl border-primary/20 bg-primary/5">
                <h3 className="text-xl font-display font-bold mb-4 text-primary flex items-center gap-2">
                  <Video className="w-5 h-5" /> Available Streams
                </h3>
                <div className="space-y-3">
                  {movie.streamUrls.map((url, idx) => (
                    ...
                    >
                      <span className="font-medium text-sm">Stream Source {idx + 1}</span>
...
// Ensure User is imported in layout but we used it in movie detail, let's make sure it's valid
